...
process Zcat_MergeFastq {
   ...
   # Extracting the Read_Fastq file paths and creating symbolic links
   str_r1=\$(echo ${Read_Fastq} | cut -d' ' -f1 | tr ',' ' ')
   str_r2=\$(echo ${Read_Fastq} | cut -d' ' -f2 | tr ',' ' ')

   ln -s \${str_r1} ${SampleName}.Merge_Read_R1.fastq.gz
   ln -s \${str_r2} ${SampleName}.Merge_Read_R2.fastq.gz
   """
}
...
process Fusion_Catcher {
   ...
   memory { (16.GB + (8.GB * task.attempt)) } // First attempt 64GB, second 96GB, etc
   errorStrategy 'retry'
   maxRetries 3
